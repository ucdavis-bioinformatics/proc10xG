...
class Barcodes:
    ...
    def read_barcode_file(self):
        ...
        try:
            ...
            sys.exit("Error: could not open file: %s" % self.file)
        else:
            with f:
                for bc_line in f:
                    try:
    ...
    def keep_barcode(self, barcode):
        ...
        if bcHash in self.gbcDict:
            val = self.gbcDict[bcHash]
            if self.min is None and self.max is None:
		# barcode in list and not specifying min/max
		return True
...
def main(read1, read2, barcode_table, output_dir, status, interleaved_in, interleaved_out, nogzip, verbose):
    ...
    try:
        ...
        sys.exit("%s unexpectedly terminated\n" % (__name__))
    except Exception:
        sys.stderr.write("".join(traceback.format_exception(*sys.exc_info())))
        sys.exit("An unknown fatal error was encountered.\n")
...
if (options.bc_min is None and options.bc_max is None) and options.barcode_file is not None:
    sys.exit("Must specify a min, or max, when specifying a barcode file")
...
if (options.bc_max < options.bc_min):
