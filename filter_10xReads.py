    ...
    try:
        while 1:
            ...
            if read_count % 250000 == 0 and verbose:
                sys.stderr.write("READS\treads analyzed:%i|reads/sec:%i|reads output:%i\n" % (read_count, round(read_count / (time.time() - stime), 0), read_output))
...
usage = "usage %prog -o [output file prefix (path + name)] -(slig) --quiet -1 [read1a,read1b] -2 [read2a,read2b]\n"
usage += "%prog will process read file produced by preocess_10xReads.py and filter for certain STATUS conditions."
...
group = parser.add_argument_group("Inputs", "Preprocessed 10x fastq files to input")

group.add_argument('-1', '--read1', metavar="read1", dest='read1', help='read1 of a pair, first processed by process_10xReads, multiple files can be specified separated by comma',
...
main(infile1, infile2, output_dir, status, interleaved_in, interleaved_out, nogzip, verbose)
