...
Modified to output exact match and 1-mismatch numbers by Jessie
"""
import traceback
import argparse
import sys
...
def main(read1, read2, output_dir, output_all, interleaved, profile, bctrim, trim, nogzip, verbose):
    ...
    gbcDict = {}
    gbcCounter = Counter()
    gbcmatCounter = Counter()
    gbcmisCounter = Counter()
    ...
    try:
        while 1:
            ...
            if seqToHash(fragment['gem_bc']) in gbcDict:  # barcode matches whitelist
                ...
                gbcmatCounter[seqToHash(fragment['gem_bc'])] += 1
                barcode_match += 1
        ...
        output.close()
        with open(output_dir + '_match_barcodes.txt', 'w') as f:
            [f.write('{0}\t{1}\n'.format(gbcDict[key], value)) for key, value in gbcmatCounter.items()]
        output.close()
        with open(output_dir + '_mismatch1_barcodes.txt', 'w') as f:
            [f.write('{0}\t{1}\n'.format(gbcDict[key], value)) for key, value in gbcmisCounter.items()]
        output.close()
